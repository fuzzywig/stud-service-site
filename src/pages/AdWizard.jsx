...
export default function AdWizard({ mode }) {
    ...
    // Admin Alert Email - Only 7 Required Fields
    const sendAdminAlert = async (advertData, userEmail, advertId) => {
        try {
            console.log('👮 Sending admin alert with 7 fields only');

            if (!advertId) {
                console.error('👮 No advertId provided');
                return;
            }
            ...
            const payload = {
                adminEmail: 'gavinoxley@gmail.com',
                userEmail: userData.email,
                userName: `${userData.firstName || ''} ${userData.lastName || ''}`.trim() || 'Pet Lover',
                advertId: advertId,
                title: advertData.title || `${advertData.breedOrType} ${advertData.intent === 'stud' ? 'Stud Service' : 'For Sale'}`,
            ...
            if (response.ok) {
                ...
                console.error('❌ Admin alert failed:', await response.json());
            }
        } catch (error) {
